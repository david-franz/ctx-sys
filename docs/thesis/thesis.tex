...
% Acronyms (kept as inline definitions)
...
% TITLE
% ============================================================================
\title{\textbf{ctx-sys: An Intelligent Context Management System for AI-Assisted Software Development}\\[0.5cm]
{\large A Hybrid Retrieval Architecture Combining Vector Embeddings, Graph Traversal, and Semantic Analysis for Scalable Code Understanding}}
...
% ============================================================================
\begin{abstract}
Modern AI coding assistants, built upon Large Language Models (LLMs), face fundamental limitations in context management. As conversations extend and codebases grow beyond context window capacities, these systems lose access to critical information---past decisions, code dependencies, documentation, and architectural context. This paper presents \textbf{ctx-sys}, an intelligent context management framework that addresses these limitations through a hybrid retrieval architecture.

The system implements a ``smart librarian'' paradigm: rather than attempting to fit all information into limited context windows, it indexes, extracts semantic meaning, and retrieves precisely relevant context on demand. The architecture integrates multiple retrieval strategies---vector similarity search using dense embeddings, graph-based traversal of code relationships, and keyword-based full-text search with BM25 ranking---unified through Reciprocal Rank Fusion (RRF).

Key contributions include: (1) a unified entity model supporting heterogeneous information types including code symbols, documentation sections, conversation history, and domain concepts; (2) multi-language AST parsing using tree-sitter for structural code understanding; (3) advanced retrieval techniques including HyDE (Hypothetical Document Embeddings) for vocabulary mismatch resolution, retrieval gating for computational efficiency, smart context expansion, and query decomposition with LLM re-ranking; (4) a draft-critique loop for hallucination detection; (5) agent-oriented memory management with explicit hot/cold tiering and checkpointing for resumable execution; and (6) overlapping chunk embeddings and incremental document updates for improved retrieval quality.

The implementation comprises approximately 20,000 lines of TypeScript with comprehensive test coverage (1,555 tests across 56 test suites spanning 10 implementation phases). The storage layer uses native SQLite via better-sqlite3 with FTS5 full-text search and BM25 ranking for high-performance local-first operation. Integration with AI assistants is achieved through the Model Context Protocol (MCP), enabling compatibility with Claude, GitHub Copilot, and similar tools. Evaluation demonstrates significant improvements in context relevance and token efficiency compared to naive full-context approaches.

\medskip
\noindent\textbf{Keywords:} Retrieval-Augmented Generation, Context Management, AI Coding Assistants, Graph RAG, Code Intelligence, Embedding Search
\end{abstract}
...
\begin{enumerate}
    ...
    \item \textbf{Advanced Retrieval Techniques}: Implementation of HyDE query expansion for vocabulary mismatch resolution, retrieval gating for computational efficiency, smart context expansion, and query decomposition with LLM re-ranking.
...
\begin{description}
    \item[Section 2] reviews related work in retrieval-augmented generation, code intelligence, and context management for LLMs.

    \item[Section 3] presents the system architecture, design principles, and component interactions.

    \item[Section 4] details the data model, database schema, and storage layer implementation.

    \item[Section 5] describes the code intelligence pipeline including AST parsing, symbol extraction, and relationship analysis.

    \item[Section 6] covers the embedding and retrieval system, including multi-strategy search and fusion algorithms.

    \item[Section 7] explains advanced retrieval techniques: HyDE expansion, retrieval gating, smart context expansion, query decomposition, and the draft-critique loop.

    \item[Section 8] discusses conversation management, session handling, and decision extraction.

    \item[Section 9] presents the agent patterns: checkpointing, memory tiering, and proactive context.

    \item[Section 10] describes the system integration through MCP, CLI, and configuration management.

    \item[Section 11] presents evaluation methodology and results.

    \item[Section 12] concludes with a discussion of limitations and future work.
\end{description}
...
Retrieval-Augmented Generation (RAG) has emerged as a fundamental paradigm for extending LLM capabilities beyond their training data. The core insight is that LLMs can effectively utilize external knowledge when provided in their context, enabling factual grounding without model retraining.

\subsubsection{Dense Retrieval}

Traditional information retrieval relied on sparse representations such as TF-IDF and BM25. The advent of dense retrieval, pioneered by systems like DPR (Dense Passage Retrieval), demonstrated that learned embeddings could significantly outperform lexical methods for semantic similarity tasks.
...
\subsubsection{Graph-Enhanced RAG}

Recent work has explored augmenting vector retrieval with knowledge graphs. Microsoft's GraphRAG demonstrated that graph structures capturing entity relationships can significantly improve retrieval quality for complex queries requiring multi-hop reasoning.
...
Query expansion addresses the vocabulary mismatch problem---users often phrase queries differently from how information is stored. HyDE (Hypothetical Document Embeddings) introduced the technique of generating hypothetical answers and using their embeddings for retrieval, which we implement as a configurable option for conceptual queries.

\subsection{Code Intelligence}

\subsubsection{Abstract Syntax Tree Analysis}

Abstract Syntax Tree (AST) parsing provides structural understanding of source code beyond surface-level text processing. Tree-sitter, a parser generator tool, has become the de facto standard for multi-language parsing in developer tools, offering incremental parsing, error recovery, and language-agnostic APIs.
...
\subsubsection{Code Search and Retrieval}
...
\begin{itemize}
    ...
    \item \textbf{Semantic}: Embedding-based similarity (CodeBERT, CodeT5)
    \item \textbf{Structural}: Graph-based representations (code2vec)
\end{itemize}
...
Automatic code summarization---generating natural language descriptions of code---enhances searchability and comprehension. We employ LLM-based summarization using the qwen3:0.6b model with specialized prompts for different symbol types (functions, classes, modules).

\subsection{Context Management for LLMs}
...
\begin{itemize}
    ...
    \item \textbf{Summarization}: Compressing history into summaries
...
Long-running AI agents require persistent memory beyond conversation windows. Systems like MemGPT introduced explicit memory hierarchies. Our hot/cold memory tiering draws inspiration from this work while providing more explicit developer control.

\subsection{Hallucination Detection}

LLM hallucinations---generating plausible but incorrect information---are particularly problematic in code generation where errors have concrete consequences. Our draft-critique loop implements self-verification, where the model critiques its own outputs against retrieved context, similar to self-consistency approaches.
...
\subsubsection{Smart Librarian Paradigm}
...
\subsubsection{Unified Entity Model}
...
\subsubsection{Strategy Plurality}
...
\subsubsection{Local-First Architecture}

The system prioritizes local execution:

\begin{itemize}
    \item Native SQLite database (via better-sqlite3) with FTS5 full-text search for storage
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        % Storage Layer
        \node[box, fill=yellow!20, below=2cm of embed] (sqlite) {SQLite + FTS5};
...
\begin{description}
    ...
    \item[VS Code Extension] (Planned) Native IDE integration with sidebar panel, hover information, and command palette access.
\end{description}
...
\begin{description}
    ...
    \item[Summarizer] Generates natural language descriptions of code symbols and document sections using LLM inference (qwen3:0.6b via Ollama).
...
\begin{description}
    \item[SQLite Database] Primary storage using better-sqlite3 (native SQLite bindings for Node.js) with FTS5 full-text search and BM25 ranking. Stores entities, metadata, sessions, and configuration.

    \item[Vector Store] Embedding vectors stored alongside entities, with cosine similarity computation for semantic search.
...
\begin{enumerate}
    ...
    \item \textbf{Embedding}: Symbol content and summaries are embedded to dense vectors, using overlapping chunk embeddings for improved retrieval coverage.
...
\begin{enumerate}
    \item \textbf{Query Analysis}: The natural language query is parsed to extract intent, keywords, and entity mentions. For complex queries, query decomposition breaks the query into sub-queries for more targeted retrieval.
    ...
    \item \textbf{Fusion and Re-ranking}: Results from all strategies are combined using Reciprocal Rank Fusion, then optionally re-ranked using LLM-based relevance scoring.

    \item \textbf{Smart Context Expansion}: Top results are expanded with surrounding context---related entities, parent scopes, and sibling definitions---to provide coherent context windows.

    \item \textbf{Context Assembly}: Results are formatted with source attribution, respecting token budgets.
...
% ============================================================================
% SECTION 4: DATA MODEL AND STORAGE
% ============================================================================
...
\subsubsection{Relationship Schema}
...
\subsubsection{Per-Project Tables}
...
CREATE VIRTUAL TABLE {prefix}_entities_fts USING fts5(
  name, qualified_name, content, summary,
  content={prefix}_entities, content_rowid=rowid
);
\end{lstlisting}
...
The system uses better-sqlite3 with native SQLite bindings, providing significantly improved performance over WebAssembly-based alternatives. Overlapping chunk embeddings ensure that content spanning chunk boundaries is properly captured in the vector space.
...
\subsubsection{Tree-sitter Integration}
...
\subsubsection{Language-Specific Extractors}
...
\paragraph{TypeScript Extractor}
...
\subsubsection{Symbol Representation}
...
\subsubsection{Relationship Graph Construction}
...
% ============================================================================
% SECTION 6: EMBEDDING AND RETRIEVAL
% ============================================================================
...
\subsubsection{Embedding Manager}
...
\subsubsection{Query Intent Detection}
...
\subsubsection{Entity Mention Extraction}
...
\subsection{Multi-Strategy Search}
...
Pattern-based matching against entity names and content, leveraging FTS5 with BM25 ranking:
...
private async keywordSearch(
  ...
  // Search with keywords using FTS5 + BM25
...
\label{sec:advanced-retrieval}

\subsection{HyDE Query Expansion}

HyDE (Hypothetical Document Embeddings) addresses the \textit{vocabulary mismatch} problem---users often phrase queries differently from how information is stored.
...
\subsubsection{Selective Application}
...
\begin{itemize}
    ...
    \item \textbf{Skipped}: Specific lookups (``find \texttt{UserController}''), short queries, debugging with exact error messages
\end{itemize}
...
Smart context expansion enriches retrieval results by automatically including surrounding context that improves comprehension.

\subsubsection{Expansion Strategy}

When a retrieved entity is a function or method, the expansion system automatically includes:

\begin{itemize}
    \item \textbf{Parent scope}: The containing class or module definition
    \item \textbf{Type dependencies}: Referenced types, interfaces, and enums
    \item \textbf{Sibling definitions}: Related functions in the same file or class
    \item \textbf{Import context}: Key imports needed to understand the code
\end{itemize}

This ensures that retrieved code snippets are self-contained and comprehensible without requiring additional round-trips.

\subsection{Query Decomposition and LLM Re-ranking}

Complex queries are decomposed into targeted sub-queries, and results are re-ranked using LLM-based relevance scoring.

\subsubsection{Query Decomposition}

Multi-faceted queries such as ``how does the authentication system validate tokens and handle expired sessions?'' are decomposed into focused sub-queries:

\begin{enumerate}
    \item ``authentication system token validation''
    \item ``expired session handling''
\end{enumerate}

Each sub-query is executed independently, and results are merged using weighted RRF with de-duplication.

\subsubsection{LLM Re-ranking}

After initial retrieval and fusion, the top-$N$ candidates are optionally re-ranked by an LLM that scores each result's relevance to the original query. This addresses cases where surface-level similarity scores fail to capture true relevance, particularly for nuanced or domain-specific queries.

\subsection{Overlapping Chunk Embeddings}

Documents and large code entities are split into overlapping chunks before embedding, ensuring that content near chunk boundaries is captured in multiple embeddings. This addresses the common problem where relevant information spanning a chunk boundary is missed by both adjacent chunk embeddings.

\subsection{Draft-Critique Loop}

The draft-critique loop verifies LLM responses against retrieved context to detect hallucinations.

\subsubsection{Hallucination Problem}
...
\subsubsection{Critique Process}
...
\subsubsection{Structured Extraction}
...
% ============================================================================
% SECTION 9: AGENT PATTERNS
% ============================================================================
\section{Agent Patterns}
\label{sec:agent-patterns}

This section describes patterns for supporting long-running AI agent workflows that exceed single conversation boundaries.

\subsection{Checkpointing}

Checkpointing enables saving and restoring agent execution state for failure recovery and task resumption.

\subsubsection{Checkpoint Data Model}
...
\subsubsection{Automatic Checkpointing}
...
\subsubsection{Access Pattern Tracking}
...
\subsubsection{Reflection Model}
...
\subsubsection{Cross-Session Learning}
...
\subsubsection{Integration with File Watching}

The proactive system integrates with the file watching infrastructure to detect relevant changes:
...
\begin{itemize}
    \item \textbf{Test Count}: 1,555 passing tests across 56 test suites spanning 10 implementation phases
...
Testing on sample codebases demonstrates:

\begin{itemize}
    \item \textbf{Keyword queries}: High precision for exact symbol lookup, enhanced by FTS5 with BM25 ranking
    \item \textbf{Conceptual queries}: Good recall with semantic search, improved by overlapping chunk embeddings
    \item \textbf{Dependency queries}: Effective graph traversal for import chains
    \item \textbf{Complex queries}: Query decomposition with LLM re-ranking significantly improves results for multi-faceted queries
\end{itemize}
...
\begin{itemize}
    ...
    \item Smart context expansion reduces follow-up queries by providing more complete context windows
    \item LLM re-ranking improves precision@10 over RRF-only fusion
\end{itemize}
...
Measurements on medium-sized codebases (10,000-50,000 lines):
\begin{itemize}
    ...
    \item Incremental document updates: sub-second for single file changes
\end{itemize}
...
\begin{itemize}
    ...
    \item FTS5 keyword search with BM25: < 5ms
...
\begin{enumerate}
    \item \textbf{Vector Search Scalability}: In-memory similarity computation limits corpus size for very large codebases. Future work may integrate dedicated vector extensions for further scaling.
...
This paper presented ctx-sys, an intelligent context management system for AI-assisted software development. The system addresses the fundamental problem of context limitations in LLM-based coding assistants through a comprehensive architecture integrating:
...
\begin{itemize}
    ...
    \item Hybrid retrieval combining vector similarity, graph traversal, and FTS5 keyword search with BM25 ranking
    \item Advanced techniques including HyDE query expansion, retrieval gating, smart context expansion, query decomposition, and LLM re-ranking
    ...
    \item Overlapping chunk embeddings and incremental document updates for improved retrieval quality
\end{itemize}
...
\begin{enumerate}
    ...
    \item Implementation of advanced patterns (HyDE, retrieval gating, draft-critique, smart context expansion, query decomposition, LLM re-ranking) adapted for code-specific use cases

    \item A complete, tested reference implementation with 1,555 passing tests across 56 test suites and MCP integration for practical deployment
\end{enumerate}
...
\subsubsection{Completed Features (Phases 1--10)}
...
\textbf{Phase 10: Performance \& Quality}
\begin{itemize}
    \item Migration from sql.js to better-sqlite3 with native FTS5 and BM25 ranking
    \item Migration from qwen2.5-coder to qwen3:0.6b for summarization
    \item Smart context expansion for enriched retrieval results
    \item Query decomposition with LLM re-ranking for complex queries
    \item Incremental document updates for efficient re-indexing
    \item Overlapping chunk embeddings for improved boundary coverage
\end{itemize}
...
This paper represents both a practical contribution (a deployable system) and a reference architecture for future work in this rapidly evolving space.
