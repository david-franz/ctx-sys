...
The implementation comprises approximately 20,000 lines of TypeScript with 1,555 tests across 56 test suites. It supports TypeScript, Python, Rust, Go, and Java, with universal document indexing for Markdown, HTML, and structured formats.
...
\begin{enumerate}
    ...
    \item \textbf{Code-aware indexing} using tree-sitter AST parsing with automatic relationship extraction across 5 programming languages
    ...
    \item A complete \textbf{reference implementation} with MCP integration, 30 tools, and production-ready deployment as an npm package
\end{enumerate}
...
\node[box, fill=white] (index) at (-0.8,3.8) {index\_codebase};
\node[box, fill=white] (memory) at (2.4,3.8) {store\_message};
\node[box, fill=white] (graph) at (5.2,3.8) {query\_graph};
\node[label] at (0,3.2) {+ 26 more tools};
...
ctx-sys uses tree-sitter to parse source code into Abstract Syntax Trees, extracting meaningful entities from five programming languages:
...
\begin{table}[H]
    ...
    \begin{tabular}{llp{8cm}}
        ...
        \midrule
        TypeScript/JS & tree-sitter-typescript & Functions, classes, methods, interfaces, type aliases, enums, imports \\
        Python & tree-sitter-python & Functions, classes, methods, decorators, imports \\
        Rust & tree-sitter-rust & Functions, structs, impls, traits, enums, imports \\
        Go & tree-sitter-go & Functions, structs, methods, interfaces, imports \\
        Java & tree-sitter-java & Classes, methods, interfaces, imports \\
        \bottomrule
    \end{tabular}
\end{table}
...
Beyond code, ctx-sys indexes documentation in any text format: Markdown, HTML, YAML, JSON, TOML, and plain text. Documents are split into semantic chunks at paragraph boundaries with configurable overlap, ensuring that each chunk is self-contained for embedding.
...
ctx-sys integrates with AI assistants through the Model Context Protocol (MCP), an open standard for connecting LLMs to external data sources. The MCP server exposes 30 tools organized into categories:

\begin{table}[H]
    \centering
    \caption{MCP tools by category}
    \label{tab:mcp-tools}
    \begin{tabular}{lll}
        \toprule
        \textbf{Category} & \textbf{Tools} & \textbf{Key Capabilities} \\
        \midrule
        Search & 2 & Hybrid RAG retrieval, entity search \\
        Indexing & 4 & Codebase indexing, document indexing, git sync \\
        Graph & 3 & Relationship traversal, graph statistics \\
        Memory & 5 & Message storage, history, session summaries \\
        Agent & 7 & Checkpoints, memory tiers, reflections \\
        Project & 4 & Project management, entity operations \\
        Git Hooks & 2 & Auto-indexing, impact analysis \\
        Other & 3 & Search decisions, index status, graph stats \\
        \bottomrule
    \end{tabular}
\end{table}
...
Qualitative assessment on real codebases demonstrates:

\begin{itemize}
    \item \textbf{Keyword queries}: High precision for exact symbol lookup, enhanced by FTS5 with BM25 ranking and camelCase identifier splitting
    \item \textbf{Conceptual queries}: Good recall with semantic search, significantly improved by multi-vector chunking of large entities
    \item \textbf{Dependency queries}: Effective graph traversal for import chains and call graphs
    \item \textbf{Complex queries}: Query decomposition with multi-strategy fusion handles multi-faceted questions well
    \item \textbf{HyDE benefit}: Measurable improvement for vocabulary-mismatched queries (e.g., ``error handling'' finding \texttt{try/catch} blocks)
\end{itemize}

Multi-strategy fusion consistently outperforms any single strategy approach. The combination of keyword precision, semantic recall, and structural understanding from graph traversal covers query types that no single strategy handles well alone.

\subsection{Performance}

\begin{table}[H]
    \centering
    \caption{Performance characteristics on medium-sized codebases (10K--50K lines)}
    \begin{tabular}{ll}
        \toprule
        \textbf{Operation} & \textbf{Performance} \\
        \midrule
        AST parsing & $\sim$100 files/second \\
        Embedding generation (Ollama) & $\sim$10 entities/second \\
        Embedding generation (OpenAI) & $\sim$50 entities/second \\
        FTS5 keyword search & $<$ 5ms \\
        Vector similarity search (sqlite-vec) & 10--50ms \\
        Graph traversal (depth 2) & 5--20ms \\
        Full retrieval pipeline & 50--200ms \\
        Full pipeline + HyDE & +500--2000ms (LLM generation) \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{enumerate}
    \item \textbf{Language support}: AST parsing limited to TypeScript, Python, Rust, Go, Java. Other languages get document-level indexing only.
    \item \textbf{Summarization latency}: LLM-based summarization is slow and should be run asynchronously or as a background task.
    \item \textbf{Embedding model dependency}: Changing embedding models requires re-embedding all entities. Model version tracking detects this but the re-embedding cost is significant for large codebases.
    \item \textbf{Single-machine}: Currently designed for single-developer use. Multi-user or team scenarios would require shared database access or replication.
\end{enumerate}
...
Retrieval-Augmented Generation (RAG) has emerged as a fundamental paradigm for extending LLM capabilities beyond training data. The core insight is that LLMs can effectively utilize external knowledge when provided in context, enabling factual grounding without model retraining.

Standard RAG implementations retrieve text chunks by vector similarity. ctx-sys extends this with graph-aware retrieval and code-specific heuristics, recognizing that code has structural relationships (imports, inheritance, calls) that flat text retrieval misses.

\subsection{Code Intelligence}

Tools like Language Server Protocol (LSP) implementations, GitHub's code search, and Sourcegraph provide structural code understanding. ctx-sys differs by focusing on \textit{retrieval for AI assistants} rather than developer navigation---the goal is to assemble relevant context within token budgets, not to provide exhaustive cross-references.

\subsection{Graph RAG}

Recent work on Graph RAG combines knowledge graphs with retrieval-augmented generation. ctx-sys builds its graph automatically from AST analysis (imports, calls, inheritance) rather than requiring manual knowledge graph construction, making it practical for any codebase without upfront investment.

\subsection{Context Management}

Systems like Mem0, LangChain Memory, and LlamaIndex provide context management for LLM applications. ctx-sys is distinguished by its focus on code-specific retrieval (AST parsing, relationship extraction), its fully local architecture, and its MCP-native integration that works with any compatible AI assistant.
